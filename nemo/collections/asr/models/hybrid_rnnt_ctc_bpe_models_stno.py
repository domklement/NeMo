...
from nemo.collections.asr.data import audio_to_text_and_stno_dataset
...
class EncDecHybridRNNTCTCBPEModelSTNO(EncDecHybridRNNTCTCModelSTNO, ASRBPEMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None, tokenizer=None):
        ...
        self.ctc_meeteval_mt_wer = MeetevalMTWER(
            decoding=self.ctc_decoding,
            dist_sync_on_step=False,
            log_prediction=self._cfg.get("log_prediction", False),
            embed_duration=self.embed_duration,
    ...
    def _setup_dataloader_from_config(self, config: Optional[Dict], val: bool = False):
        ...
        dataset = audio_to_text_and_stno_dataset.get_audio_to_text_bpe_dataset_from_config(
            config=config,
    ...
    def change_vocabulary(
        ...
        del self.joint
        self.joint = EncDecHybridRNNTCTCBPEModelSTNO.from_config_dict(new_joint_config)

        decoder_config = self.decoder.to_config_dict()
        new_decoder_config = copy.deepcopy(decoder_config)
        new_decoder_config.vocab_size = len(vocabulary)
        del self.decoder
        self.decoder = EncDecHybridRNNTCTCBPEModelSTNO.from_config_dict(new_decoder_config)
        ...
        if hasattr(self, 'ctc_decoder'):
            ...
            self.ctc_decoder = EncDecHybridRNNTCTCBPEModelSTNO.from_config_dict(ctc_decoder_config)
            del self.ctc_loss
