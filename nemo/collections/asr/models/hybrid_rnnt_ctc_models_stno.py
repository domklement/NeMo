...
class EncDecHybridRNNTCTCModelSTNO(EncDecRNNTModelSTNO, ASRBPEMixin, InterCTCMixin, ASRTranscriptionMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.ctc_decoder = EncDecRNNTModelSTNO.from_config_dict(self.cfg.aux_ctc.decoder)
    ...
    def change_vocabulary(
        ...
        if hasattr(self, 'ctc_decoder'):
            if self.ctc_decoder.vocabulary == new_vocabulary:
                ...
                self.ctc_decoder = EncDecHybridRNNTCTCModelSTNO.from_config_dict(new_decoder_config)
                del self.ctc_loss
                self.ctc_loss = CTCLoss(
                    num_classes=self.ctc_decoder.num_classes_with_blank - 1,
                    zero_infinity=True,
    ...
    def validation_pass(self, batch, batch_idx, dataloader_idx):
        ...
        self.ctc_meeteval_mt_wer.update(
            predictions=log_probs,
            predictions_lengths=encoded_len,
            utt_ids=utt_ids,
            spk_ids=spk_ids,
    ...
    def on_validation_epoch_end(self, sync_metrics: bool = False) -> Optional[Dict[str, Dict[str, torch.Tensor]]]:
        ...
        if isinstance(self.validation_step_outputs[0], dict):
            ...
            ctc_cp_res, ctc_tcp_res = self.ctc_meeteval_mt_wer.compute(self._validation_dl.dataset.manifest_processor.collection)
            output_dict['log'].update({'val/ctc_cp_wer': ctc_cp_res['wer'], 'val/ctc_cp_ins': ctc_cp_res['ins'], 'val/ctc_cp_del': ctc_cp_res['del'], 'val/ctc_cp_sub': ctc_cp_res['sub'], 'val/ctc_cp_len': ctc_cp_res['len'],
                                    'val/ctc_tcp_wer': ctc_tcp_res['wer'], 'val/ctc_tcp_ins': ctc_tcp_res['ins'], 'val/ctc_tcp_del': ctc_tcp_res['del'], 'val/ctc_tcp_sub': ctc_tcp_res['sub'], 'val/ctc_tcp_len': ctc_tcp_res['len']})
            self.ctc_meeteval_mt_wer.reset()
    ...
    # EncDecRNNTModelSTNO is exported in 2 parts
