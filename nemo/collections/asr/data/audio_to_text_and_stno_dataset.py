...
from nemo.collections.asr.data import audio_to_text_dali, audio_to_text_and_stno
...
def get_char_dataset(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> audio_to_text_and_stno.AudioToCharDataset:
    """
    ...
    dataset = audio_to_text_and_stno.AudioToCharDataset(
...
def get_bpe_dataset(
    ...
    """
    dataset = audio_to_text_and_stno.AudioToBPEAndSTNODataset(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
def get_tarred_dataset(
    ...
    tokenizer: Optional['TokenizerSpec'] = None,
    augmentor: Optional['AudioAugmentor'] = None,
) -> Union[audio_to_text_and_stno.TarredAudioToBPEDataset, audio_to_text_and_stno.TarredAudioToCharDataset]:
    """
    ...
    for dataset_idx, (tarred_audio_filepath, manifest_filepath) in enumerate(
        ...
        if tokenizer is None:
            dataset = audio_to_text_and_stno.TarredAudioToCharDataset(
            ...
            dataset = audio_to_text_and_stno.TarredAudioToBPEDataset(
                audio_tar_filepaths=tarred_audio_filepath,
                manifest_filepath=manifest_filepath,
                tokenizer=tokenizer,
                sample_rate=config['sample_rate'],
...
def get_chain_dataset(datasets, ds_config, rank=0):
    if len(datasets) > 1:
        if ds_config.get('bucketing_batch_size', None) is not None:
            ...
            for idx, dataset in enumerate(datasets):
                datasets[idx] = audio_to_text_and_stno.BucketingDataset(
    ...
    if bucketing_strategy == 'fixed_order':
        ...
        return audio_to_text_and_stno.RandomizedChainDataset(datasets=datasets, rnd_seed=0)
    elif bucketing_strategy == 'fully_randomized':
        return audio_to_text_and_stno.RandomizedChainDataset(datasets=datasets, rnd_seed=random.randint(0, 30000) + rank)
    else:
        raise ValueError(
