...
class PITLossWrapper(nn.Module):
    ...
    def forward(
        self, est_targets, targets, target_lens, return_est=False, reduce_kwargs=None, **kwargs
    ):
        ...
        if self.pit_from == "pw_mtx":
            ...
            pw_losses = self.get_pw_losses(
                self.loss_func, est_targets, targets, target_lens, **kwargs
            )
    ...
    @staticmethod
    def get_pw_losses(loss_func, est_targets, targets, target_lens, **kwargs):
        ...
        for est_idx, est_src in enumerate(est_targets.transpose(0, 1)):
            for target_idx, target_src in enumerate(targets.transpose(0, 1)):
                pair_wise_losses[:, est_idx, target_idx] = loss_func(probs=est_src.unsqueeze(-1), labels=target_src.unsqueeze(-1), target_lens=target_lens)
