...
class AVTextSTNO(_Collection):
    """List of audio-transcript text correspondence with preprocessing."""

    OUTPUT_TYPE = collections.namedtuple(
        typename='AudioTextEntity',
        field_names='id audio_file duration text_tokens offset text_raw speaker orig_sr lang per_spk_feature_files',
    )
    ...
    def __init__(
        ...
        token_labels: List[Optional[int]],
        langs: List[Optional[str]],
        per_spk_feature_files_list: List[Dict[str, str]],
        parser: parsers.CharParser,
        min_duration: Optional[float] = None,
        max_duration: Optional[float] = None,
        max_number: Optional[int] = None,
        ...
        for id_, audio_file, duration, offset, text, speaker, orig_sr, token_labels, lang, per_spk_feature_files in zip(
            ids, audio_files, durations, offsets, texts, speakers, orig_sampling_rates, token_labels, langs, per_spk_feature_files_list
        ):
            if duration is None:
                all_has_duration = False
            # Duration filters.
            ...
            data.append(output_type(id_, audio_file, duration, text_tokens, offset, text, speaker, orig_sr, lang, per_spk_feature_files))
            if index_by_file_id:
                file_id, _ = os.path.splitext(os.path.basename(audio_file))
                if file_id not in self.mapping:
                    self.mapping[file_id] = []
...
class ASRAVTextSTNO(AVTextSTNO):
    """`AudioText` collector from asr structured json files."""
    ...
    def __init__(self, manifests_files: Union[str, List[str]], parse_func: Optional[Callable] = None, *args, **kwargs):
        ...
        for item in manifest.item_iter(manifests_files, parse_func=parse_func):
            ...
            token_labels.append(item['token_labels'])
            langs.append(item['lang'])
            per_spk_feature_files.append(item['per_spk_feature_files'])
        super().__init__(
            ids, audio_files, durations, texts, offsets, speakers, orig_srs, token_labels, langs, per_spk_feature_files, *args, **kwargs
        )
