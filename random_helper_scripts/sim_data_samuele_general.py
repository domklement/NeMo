...
# note that speaker position may change.

RT60 = (0.1, 0.4)
ROOM_SZ = (5, 10)
ROOM_CEILING = (3, 5)
DELTA_DIST = 0.5  # min distance between walls and among speakers
RAN_DISP = 0.05 # better probably to disable for multi channel
BASE_GAIN = (-35, -8)
REL_GAIN = (-12, +5)

SAVE_SPK = False # it will save the reverberated signal.
# do we want to save also the dry signal ? helpful to chyiu jiunn probably.
# but then we need to align and also use fftconvolve on the dry.

MULTICHANNEL = False #TODO need also to estimate the direct path probably.
ADD_NOISE = False
SAVE_CUTSET = False
...
class ConversationalMeetingSimulator:
    ...
    def get_offset(self, prev_cut, prev_offset, prev_offset_spk, transition_type):
        ...
        if transition_type == TransitionType.TURN_HOLD:
            ...
            return max(prev_offset + prev_cut.duration + pause_duration, prev_offset_spk)
            ...
            return max(prev_offset + prev_cut.duration - overlap_duration, prev_offset_spk)
    ...
    def gen_audio(self, indx):
        ...
        while current_time < target_dur or len(seen_speakers) < len(sampled_spk):
            ...
            if utt_indx == 0:
                ...
                if not c_spk in prev_offset_spk.keys():
                    c_offset = self.get_offset(prev_cut, prev_offset, 0.0, transition_type)
                else:
                    c_offset = self.get_offset(prev_cut, prev_offset, prev_offset_spk[c_spk], transition_type)
            ...
            if prev_speaker not in prev_offset_spk.keys():
                ...
                prev_offset_spk[prev_speaker] = max(prev_offset + cut.duration, prev_offset_spk[prev_speaker])
        ...
        for cut, offset, c_speech_lvl in zip(utterances, offsets, speech_lvls):
            ...
            if (offset + c_audio.shape[-1]) > maxlen:
                ...
                if self.save_spk:
                    ...
                    (spk2audio[c_spk], c_audio[:, -residual:]), axis=-1
                )
        ...
        if self.save_spk:
            for k in spk2audio:
                sf.write(os.path.join(self.output_dir, f"{recording_id}-spk-{k}.wav"), spk2audio[k].T, self.sample_rate)
        ...
        for i, (cut, offset) in enumerate(zip(utterances, offsets)):
            ...
            # Adjust alignments if they exist #TODO

            try:
                orig_supervision.alignment.keys()
            except:
                import pdb
                pdb.set_trace()


            if hasattr(orig_supervision, "alignment") and "word" in orig_supervision.alignment.keys():
            ...
            supervision = SupervisionSegment(
                ...
                alignment={"word": alignment} if alignment is not None else None,  # Include adjusted alignments
                custom={
...
if __name__ == "__main__":
    ...
    rand_seed = num_from + random.randint(0, num_to-num_from)
    ...
    #         output_dir="/scratch/project/open-33-6/dklement/data/diar_data/rirs",
    ...
    if STAGE <= 4:
        ...
        try:
            ...
            all_cuts = lhotse.load_manifest(
                os.path.join(
                    OUTPUT_DIR, "manifests", "all_cuts_splitted.jsonl.gz"
                )
            )
            logger.info("Source CutSet loaded")

        rirs = lhotse.load_manifest(RIR_PATH)
        tmp = []
